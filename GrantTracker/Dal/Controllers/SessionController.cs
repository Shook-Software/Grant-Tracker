...
public class SessionController : ControllerBase
{
    ...
    public async Task<ActionResult<List<AttendanceConflict>>> VerifyAttendance(Guid sessionGuid, string instanceDate, [FromBody] List<StudentAttendanceDto> studentRecords)
    {
        try
        {
            ...
            if (session.SessionType.Label != "Parent")
                conflicts = await _attendanceRepository.ValidateStudentAttendanceAsync(DateOnly.Parse(instanceDate), studentRecords);
